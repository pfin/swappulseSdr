 ...
 * Analytics Page
 * 
 * Page for visualizing DTCC trade data analytics.
 */
  ...
  const handleFetchData = async (params: DTCCFetchParams) => {
    ...
    try {
      ...
      if (analyticsResponse.ok) {
        ...
        throw new Error(errorData.error || 'Failed to generate analytics');
      }
    } catch (err) {
  ...
  return (
    <Layout>
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">DTCC Analytics Dashboard</h1>
          <p className="mt-2 text-gray-600">
            Visualize and analyze swap trade data from the DTCC Swap Data Repository.
          </p>
        </div>
            ...
            <div className="flex">
              ...
              <div className="ml-3">
                <p className="text-sm text-blue-700">
                  Analyzed {metadata.count.toLocaleString()} trades from {new Date(metadata.startDate).toLocaleDateString()} to {new Date(metadata.endDate).toLocaleDateString()}
                </p>
              </div>
            </div>
          </div>
